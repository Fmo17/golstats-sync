/**
 * testar-media-gols.js
 *
 * Testa a MÉDIA entre o indicador de ataque (gols feitos) e o de defesa
 * (gols sofridos) pro mercado de gols -- em vez de exigir que os 2
 * apontem pra EXATAMENTE a mesma faixa (consenso rígido, que descartou
 * 86% dos jogos no teste anterior), combina os 2 diffPercentual numa
 * média só, e classifica essa média nas mesmas faixas de sempre.
 *
 *   diffCombinado = (diffAtaque + diffDefesa) / 2
 *
 * Mantém praticamente todo o volume de sinais (só precisa que os 2
 * indicadores existam, não que concordem exatamente).
 *
 * Uso:
 *   node scripts/testar-media-gols.js [--competicao=71] [--detalhado] [--limite=50]
 */
...
async function main() {
  ...
  const pesoAtaque = pesoAtaqueArg ? parseFloat(pesoAtaqueArg.split('=')[1]) : 0.5;
  ...
  let totalJogosComOsDoisIndicadores = 0;
  ...
  for (const comp of competicoesParaTestar) {
    ...
    for (let i = 0; i < partidas.length; i++) {
      ...
      totalJogosComOsDoisIndicadores++;
  ...
  console.log(`\n\n=== RESULTADO GLOBAL DA MÉDIA (peso ataque=${pesoAtaque}, defesa=${pesoDefesa.toFixed(2)}) ===`);
  ...
  console.log(`\n  Jogos com os 2 indicadores calculáveis: ${totalJogosComOsDoisIndicadores}`);
  console.log(`  Desses, com sinal gerado (faixa != null): ${totalComFaixa} (${((totalComFaixa / totalJogosComOsDoisIndicadores) * 100).toFixed(1)}%)`);
