/**
 * testar-consenso-gols.js
 *
 * Testa o CONSENSO entre o indicador de ataque (gols feitos) e o de defesa
 * (gols sofridos) pro mercado de gols -- só considera sinal quando os 2
 * apontam pra MESMA faixa (1+, 2+ ou 3+) ao mesmo tempo.
 *
 * Compara contra todo o histórico real, mostrando: quantos sinais o
 * consenso gera (bem menos que cada indicador sozinho, já que exige
 * concordância), e a taxa de acerto desses sinais mais rigorosos.
 *
 * Uso:
 *   node scripts/testar-consenso-gols.js [--competicao=71] [--detalhado] [--limite=50]
 */
...
// ---------- Indicador de ataque (gols feitos) -- igual ao que já está em produção ----------
...
// ---------- Indicador de defesa (gols sofridos) -- o que acabamos de validar ----------
function calcularDiffPercentualDefesa(partidasAnteriores, timeCasaId, timeForaId) {
  const jogosMandante = partidasAnteriores.filter((p) => p.time_casa_id === timeCasaId).slice(-JANELA_GOLS);
  if (jogosMandante.length < MINIMO_JOGOS_TIME) return null;
...
async function main() {
  ...
  const resultadosGlobaisConsenso = { '3mais': { total: 0, acertos: 0 }, '2mais': { total: 0, acertos: 0 }, '1mais': { total: 0, acertos: 0 } };
  let totalJogosTestados = 0;
  let totalAtaqueSozinho = 0;
  let totalDefesaSozinha = 0;
  ...
  for (const comp of competicoesParaTestar) {
    ...
    for (let i = 0; i < partidas.length; i++) {
      ...
      const diffDefesa = calcularDiffPercentualDefesa(historicoAntes, partida.time_casa_id, partida.time_fora_id);
      ...
      if (faixaAtaque && !faixaDefesa) totalAtaqueSozinho++;
      if (faixaDefesa && !faixaAtaque) totalDefesaSozinha++;

      // CONSENSO: só conta quando as 2 faixas existem E são iguais
      if (!faixaAtaque || !faixaDefesa) continue;
      if (faixaAtaque !== faixaDefesa) { totalDiscordam++; continue; }

      totalConcordam++;
      const faixaConsenso = faixaAtaque;
      const golsTotais = partida.gols_casa + partida.gols_fora;
      const acertou = bateuFaixa(faixaConsenso, golsTotais);

      resultadosComp[faixaConsenso].total++;
      resultadosGlobaisConsenso[faixaConsenso].total++;
      if (acertou) { resultadosComp[faixaConsenso].acertos++; resultadosGlobaisConsenso[faixaConsenso].acertos++; }
  ...
  console.log('\n\n=== RESULTADO GLOBAL DO CONSENSO ===');
  for (const [faixaNome, dados] of Object.entries(resultadosGlobaisConsenso)) {
    if (dados.total === 0) { console.log(`  ${faixaNome}: sem casos`); continue; }
    const taxa = (dados.acertos / dados.total) * 100;
    const aviso = dados.total < 60 ? '  ⚠️  amostra pequena' : '';
    console.log(`  ${faixaNome}: ${dados.acertos}/${dados.total} (${taxa.toFixed(1)}%)${aviso}`);
  ...
  console.log('\n=== Quanto o consenso "filtra" comparado a usar 1 indicador sozinho ===');
  console.log(`  Jogos testados (com dado suficiente pros 2 indicadores): ${totalJogosTestados}`);
  console.log(`  Ataque e defesa CONCORDAM na mesma faixa: ${totalConcordam} (${((totalConcordam / totalJogosTestados) * 100).toFixed(1)}%)`);
  console.log(`  Ataque e defesa DISCORDAM (faixas diferentes): ${totalDiscordam} (${((totalDiscordam / totalJogosTestados) * 100).toFixed(1)}%)`);
  console.log(`  Só ataque deu faixa (defesa não): ${totalAtaqueSozinho}`);
  console.log(`  Só defesa deu faixa (ataque não): ${totalDefesaSozinha}`);
  ...
  if (detalhado) {
    ...
    console.log(`\n\n=== Detalhe jogo a jogo (${maisRecentes.length} mais recentes, de ${todosOsJogosDetalhados.length} com consenso) ===\n`);
    for (const j of maisRecentes) {
      ...
      console.log(`   Previsto (consenso): ${rotulos[j.faixa]}  |  ataque: ${j.diffAtaque.toFixed(1)}%, defesa: ${j.diffDefesa.toFixed(1)}%  |  Gols reais: ${j.golsCasa + j.golsFora}\n`);
    }
  }
}
